...
namespace Vladify.Tests.UnitTests.ModerationTasks.Commands.ApproveTask;

public class ApproveTaskCommandTests
{
    private readonly IFixture _fixture;
    private readonly Mock<IModerationTaskRepository> _repositoryMock;
    private readonly Mock<IMapper> _mapperMock;
    private readonly ApproveTaskCommandHandler _handler;

    public ApproveTaskCommandTests()
    {
        _fixture = new Fixture().Customize(new AutoMoqCustomization());
        _repositoryMock = _fixture.Freeze<Mock<IModerationTaskRepository>>();
        _mapperMock = _fixture.Freeze<Mock<IMapper>>();
        _handler = _fixture.Create<ApproveTaskCommandHandler>();
    }

    [Fact]
    public async Task Handle_ShouldApproveTaskAndReturnResponse_WhenTaskExistsAndAssignedToCorrectModerator()
    {
        ...
        var expectedResponse = _fixture.Create<ApprovedTaskResponse>();
        ...
        _mapperMock
            .Setup(m => m.Map<ApprovedTaskResponse>(updatedTask))
        ...
        existingTask.Status.Should().Be(ModerationStatus.Approved);
